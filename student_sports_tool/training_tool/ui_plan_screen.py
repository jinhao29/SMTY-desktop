...
class WeeklyTab(QWidget):
    ...
    def _init_ui(self):
        ...
        hint_row.addStretch()
        # v26.2：原右列操作面板融合进单次页左列后，周计划页补齐导出/发送入口（由 main 接线）
        self.btn_export = QPushButton('⤓ 导出 Excel', objectName='primary')
        self.btn_export.setCursor(Qt.PointingHandCursor)
        self.btn_export.setMinimumHeight(30)
        hint_row.addWidget(self.btn_export)
        self.btn_send_phone = QPushButton('截图发手机', objectName='secondary')
        self.btn_send_phone.setCursor(Qt.PointingHandCursor)
        self.btn_send_phone.setMinimumHeight(30)
        hint_row.addWidget(self.btn_send_phone)
        tl.addLayout(hint_row)
